...
const RenameWallet = () => {
  ...
  const renameWallet = async (renamedName: string) => {
    if (wallets.map((i) => i.name).includes(renamedName)) return toast.error("This name is already token");
  ...
  }, [setWallet, wallets, walletId, wallet]);
  ...
  return (
    <div className={s.renameWallet}>
      <Rename handler={renameWallet} oldName={wallet.name} />
    </div>
  );
};
